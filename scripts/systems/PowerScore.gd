...
static func item_power(item_id: String) -> float:
	...
	var p := 0.0
	# 数值属性按色阶缩放（item_stat）；副属性/光环不吃色阶，原样计分。
	p += Backpack.item_stat(item_id, "atk")   * W_ATK
	p += Backpack.item_stat(item_id, "magic") * W_MAGIC
	p += Backpack.item_stat(item_id, "def")   * W_DEF
	p += Backpack.item_stat(item_id, "hp")    * W_HP
	p += Backpack.item_stat(item_id, "mp")    * W_MP
