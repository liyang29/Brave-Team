...
**触及规则**（`_soft_reachable`）：
- **近战单位（战士/牧师，非 `can_reach_back`）只能打对方前排**——打不到后排。
- **远程单位（法师/弓手/盗贼，及 `is_ranged` 的怪）可打任何人**（含后排）。
- 配合"前排全灭→后排顶上"：近战永远有前排可打，不会卡死；想点掉对方后排，得靠你的远程。
- → **战术含义**：近战啃前线、远程狙后排；把脆皮藏后排只防得住对方近战，对方远程仍能够到。
