...
func test_record_layer_below_threshold_unlocks_nothing() -> void:
	var newly: Array = MetaProgress.record_layer(7)   # 最早的门槛(book_cleave)是层8
	assert_true(newly.is_empty(), "第7层没有任何解锁项达标")
	assert_false(MetaProgress.is_unlocked("book_cleave"), "仍锁定")

func test_record_layer_at_threshold_unlocks() -> void:
	var newly: Array = MetaProgress.record_layer(8)
	assert_true("book_cleave" in newly, "第8层达标 → 横扫书本次新解锁")
	assert_true(MetaProgress.is_unlocked("book_cleave"), "解锁状态生效")

func test_record_layer_unlocks_everything_up_to_that_layer() -> void:
	MetaProgress.record_layer(25)
	assert_true(MetaProgress.is_unlocked("book_cleave"), "层8门槛已达标")
	assert_true(MetaProgress.is_unlocked("book_taunt"), "层12门槛已达标")
	assert_true(MetaProgress.is_unlocked("book_icelance"), "层12门槛已达标")
	assert_true(MetaProgress.is_unlocked("book_purify"), "层16门槛已达标")
	assert_true(MetaProgress.is_unlocked("rogue"), "层25门槛已达标")
	assert_false(MetaProgress.is_unlocked("archer"), "层35门槛未达标 → 仍锁定")

func test_record_layer_does_not_regress_or_retrigger() -> void:
	MetaProgress.record_layer(25)
	var newly: Array = MetaProgress.record_layer(8)   # 比历史最深浅 → 无操作
	assert_true(newly.is_empty(), "比历史最深浅的层数不触发（也不返回重复解锁）")
	assert_eq(MetaProgress.best_layer_ever, 25, "历史最深纪录不倒退")

func test_record_layer_only_returns_newly_unlocked_this_call() -> void:
	MetaProgress.record_layer(8)    # 解锁 book_cleave
	var newly: Array = MetaProgress.record_layer(12)   # 再深一层
...
func test_locked_summary_excludes_unlocked() -> void:
	...
	MetaProgress.record_layer(8)   # 解锁 book_cleave 一项
...
func test_save_and_load_roundtrip() -> void:
	MetaProgress.record_layer(22)   # 解锁一批
...
func test_recruit_pool_includes_unlocked_classes_after_unlock() -> void:
	MetaProgress.record_layer(25)   # 解锁盗贼；record_layer(0)(start_run内部调) 不会让纪录倒退
...
func test_loot_pool_includes_unlocked_items_after_unlock() -> void:
	...
	var saw_it := false
	for i in range(80):
		var draft: Array = LootTable.draw_draft(3, 8)
