...
- [x] **Boss 机制（通用引擎，治"顶级 build 让魔王形同虚设"）**：三次独立压测（好build/多书连招/色阶合成）都测出同一个洞——魔王打不过顶级 build。解法是**通用可复用的 Boss 引擎**，不是一个个 Boss 手搓类。① **地图拉长到 45 层**（`MapConfig.layers` 9→45），新增 `mid_boss_layers:[20,30,40]` 三个中程 Boss 层；**现有 5 张按层门槛表按比例重标定**（meta 解锁/装备 min_layer/battle-elite 分档/深度缩放系数/掉落色阶曲线），否则新增的 30 多层会是"数值真空"。② `EncounterData.gd`（新文件，R6 最小子集）：只解决"这场战斗的 Boss 是谁、有没有阶段转换/召唤援军"，用 `MID_BOSS_PROFILES`（层→档案）数据表；`BattleSimulator.simulate()` 加可选第三参数 `boss_config:Dictionary={}`（缺省空=普通遭遇，向后兼容零改动）。③ **阶段转换引擎**：`BattleCombatant` 加 `available_skills/boss_phases/boss_phase_index` 字段（默认空，非 Boss 单位零影响）；`_check_boss_phase()` 在受伤后检查血量是否跌破下一阈值，跌破则属性乘倍率+解锁新技能（`while` 循环支持一次重创连续跨阶）；接入普攻/AOE/单体技能/DoT 四处伤害结算点。④ **召唤援军引擎**：`BattleCombatant.boss_summons`（`{every,group,max_total}` 规则数组）；`_check_boss_summons()` 每回合开始检查，按 `MonsterFactory.create_group` 生成新单位加入战场，封顶 `max_total`（含"名额只剩一半"的部分召唤）。⑤ **BossStrategy**（新策略类）：从 `available_skills` 里挑可放技能随机放（保持敌方"不可预测"传统），不像英雄连招那样确定性；集火最低血量目标。⑥ **3 个 Boss 数据配置**演示三种组合：石卫王(20层,纯阶段转换)/毒沼领主(30层,纯召唤援军)/深渊统领(40层,两者都要)；新增 3 只怪(`MonsterFactory`)+4 个 Boss 技能(`SkillTable`)。⑦ `MapGenerator` 在固定锚点步骤强制生成 `mid_boss` 单节点 funnel（同 `pre_boss`/`boss` 套路，col 恒 0），`_fill_content` 从 `EncounterData` 拉数据；`Encounter.gd` 把节点的 `boss_config` 传给 `simulate()`。**魔王 `demon_lord` 保留为唯一终点/胜利条件**，中程 Boss 只是加压检查点，不触发胜利。配 GUT（`test_boss_mechanics` 20 条：注入/阶段/召唤/策略/端到端 + `test_map_generator` 追加 3 条节点放置校验）；302/302 绿。**平衡压测**（`test_mid_boss_pressure`，纯打尺子非精确经济模拟）：同一 build 不合成升阶时三个 Boss 全 0/20，配上粗略匹配层数的合成升阶（tier1/2/3）后全 20/20——证明阶段/召唤机制确实吃"是否跟上装备节奏"、不是摆设，**精确的每层经济节奏仍需后续真实游玩数据校准**。
