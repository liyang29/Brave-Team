...
- [x] **村庄商店（节点类型 `shop`）**：开局第 0 节点=村庄，起步金币 500；按 rarity 随机上货 6 件、定价 50/120/250、只买不卖（`ShopScreen`+`Shop` 场景，RunManager `SHOP` 状态/`buy_item`/`leave_shop`）。解决"空背包首战无装备"死锁；去掉了"必须摆装备才能开战"限制（允许裸打）。
...
- [x] **英雄池 + 酒馆招募（节点类型 `tavern`）**：`HERO_TEMPLATES` 英雄池（5 职业含盗贼/猎人，加英雄=加一行）；酒馆上 3 候选、120 金/人、队伍上限 5，新人满血空背包自动站位（`TavernScreen`+`Tavern` 场景，RunManager `TAVERN` 状态/`recruit`/`leave_tavern`）。地图：村庄→林间→**酒馆**→剧毒→泉水→废墟→魔王。
