...
- [ ] **【下一步】休息/泉水回血点（Step 6）**：节点加 `"rest"` 类型回血。**回血点是"不回血消耗战"的泄压阀、必需品**（TODO Step 6）。← 下一步（搭配 Step 5 首战平衡）
- [ ] **【再下一档】事件节点 + 分支地图**：分支地图（杀戮尖塔式）把战斗/精英/泉水/商店/事件串成有得选的路。商店已可复用为节点类型。旅程层保持轻（只做深背包一个系统）。
