...
## 7. 站位（世界树式，`positioning_mode="soft_row"`）

**触及规则**（`_soft_reachable`，模型 C＝站位 + 职业）：
- **前排单位：可打对方任何排**（顶在前面就能威胁敌方后排）。
- **后排单位**：远程（法师/弓手，及 `is_ranged` 的怪）可打任何人；**近战（战士/牧师/盗贼）只能打对方前排**。
  - 英雄远程身份按职业，列在 `BattleSimulator.REACH_BACK_CLASSES`（目前仅 法师/弓手）；盗贼是近战刺客（不在列）。
- 配合"前排全灭→后排顶上"：近战永远有前排可打，不会卡死。
- → **战术含义**：想威胁对方后排，要么把近战顶到前排、要么用远程；脆皮藏后排只防得住对方"后排近战"，对方前排和远程仍够得到。
...
- **AI 不按"有效伤害"选目标**：远程能打后排，但选目标只看职业本能（血最多/防最低…），可能跑去点一个被站位打 7 折的后排肉盾，而非按"基础×站位倍率"挑最优。治本优化（`choose_target` 算有效伤害）尚未做。
- **后排远程攻击者仍吃 ×0.5 输出惩罚**：`_row_damage_mult` 按物理/魔法判定，后排弓手物理攻击虽能触及后排，但输出 ×0.5（设定为"在后方出手吃力"，可接受；若要远程无视该惩罚需再调）。
